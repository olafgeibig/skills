...
def main():
    ...
    if args.command == "query":
        ...
        try:
            ...
            # LightRAG server implementations differ:
            # - some return "context" (string)
            # - some return "contexts" (list)
            # - some may return "context" as list, or nest under "data"
            # Be defensive so --only-context is usable across servers.
            if args.only_context:
                ctx = data.get("context")
                if ctx is None:
                    ctx = data.get("contexts")
                if ctx is None and isinstance(data.get("data"), dict):
                    ctx = data["data"].get("context") or data["data"].get("contexts")

                if isinstance(ctx, list):
                    # Join list entries into a readable block
                    ctx = "\n\n".join(str(x) for x in ctx if x is not None)

                if ctx:
                    print(ctx)
                else:
                    # Fall back to response so the user still gets something useful
                    print(data.get("response", "No context found."))
            else:

...
1. Put generic domain-wiki architecture, ingest, query, source, and lint behavior in this skill.
2. Put generic vault structure and MoC guidance in `vault-ops`; put TurboVault tool mechanics in `turbovault-use`.
3. Put reusable project-specific procedure in the appropriate `project-*` skill.
4. Put wiki-local schemas and taxonomy in the domain wiki's `SCHEMA.md`, project facts in project content, and profile quirks in profile guidance.
5. Never edit a third-party skill directly.
